...
contract GameToken is ERC721, Ownable, ERC2771Recipient {
    ...
    constructor() ERC2771Recipient() ERC721("ATBToken", "ATBT") {
        // create parent of all tokens
        CreateToken(0, 0, 0, 0, 0);
    }
    
    // цей метод має викликатись сервером для створення НФТ при скануванні QR- коду. 
    function mint(uint256 _genes, uint8 _charge) external onlyOwner {
        CreateToken(0, 0, 0, _genes, _charge);
    } 

    function Breeding(uint256 token1, uint256 token2, uint256 blockNumber) external onlyOwner returns(uint256) {
        ...
        Token memory _newToken = IBreedScience(_BreedScience).mixGenes(token1, mom, token2, dad, blockNumber);
        ...
        //todo implement IndexOf function for arrays
        uint256 _newTokenId = Tokens.length == 0 ? 0 : Tokens.length - 1;

        _mint(owner(), _newTokenId);

        return _newTokenId;
    }

    function CreateToken(uint256 _parent1, uint256 _parent2, uint8 _generation, uint256 _genes, uint8 _charge) internal returns(uint256){
